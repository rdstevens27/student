...
Implement Student, Instructor, Major class and read line to line data from the file and store in a repository
"""
...
class Student:
    ...
    def p_student(self):
        ...
        return [self.cwid, self.name, self.major, sorted(self.stud_courses.keys()), self.missing_req_courses,
                self.missing_ele_courses]
...
class Repository:
    ...
    def analyze_majors(self):
        """ Adding the major deatils into a container """
        if not os.path.exists(self.directory):
            raise FileNotFoundError("No such directory")
    ...
    def major_table(self):
        """ Print the major data in a pretty table """
